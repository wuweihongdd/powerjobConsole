  ...
  <div id="job_manager">
    ...
    <el-container>
      ...
      <el-aside width="200px" style="border-right: 1px solid #ebeef5;">
        <el-row>
          <el-col :span="12">
            <el-button @click="addNode" type="primary">+</el-button>
          </el-col>
        </el-row>
        <el-tree
            :data="treeData"
      ...
      <el-main>
        ...
        <el-row>
          <el-table :data="jobInfoPageResult.data" style="width: 100%">
            ...
            <el-table-column :label="$t('message.operation')" width="150">
              <template slot-scope="scope">
                <el-button size="mini" type="text" @click="onClickModify(scope.row)">{{ $t('message.edit') }}
                </el-button>
        ...
        <el-dialog :close-on-click-modal="false" :visible.sync="modifiedJobFormVisible" width="80%">
          <el-form :model="modifiedJobForm" label-width="120px">
            ...
            <el-form-item :label="$t('message.workerConfig')">
              <el-row>
                <el-col :span="8">
                  <el-input :placeholder="$t('message.minCPU')" v-model="modifiedJobForm.minCpuCores"
                            class="ruleContent">
                ...
                <el-col :span="8">
                  <el-input :placeholder="$t('message.minDisk')" v-model="modifiedJobForm.minDiskSpace"
                            class="ruleContent">
        ...
        <!-- 任务导入导出 -->
        <el-dialog :close-on-click-modal="false" :visible.sync="jobExporterDialogVisible"
        ...
        >
          ...
          <span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="onClickRun(temporaryRowData)"
...
export default {
  ...
  data() {
    return {
      ...
      // 分发类型
      dispatchStrategy: [{key: 'HEALTH_FIRST', label: 'HEALTH_FIRST'}, {
        key: 'RANDOM',
        label: 'RANDOM'
  ...
  methods: {
    renderContent(h, { node, data }) {
      let isRoot = data.id == null || data.id == '';
      return isRoot ? (
          ...
          <span class="custom-tree-node">
            <span>{node.label}</span>
            <span v-if="node.id != null && node.id!=''">
              <el-button size="mini" type="text" icon="el-icon-edit" on-click={ () => this.openEditDialog(data) }></el-button>
              <el-button size="mini" type="text" icon="el-icon-delete"  on-click={ () => this.removeNode(node, data) }></el-button>
            </span>
          </span>);
    },
    ...
    removeNode(node, data) {
      console.log(data);
      this.axios.post("/job/catalog/delete", {id: node.id}).then(() => {
        this.$message({
          type: 'success',
          message: "删除成功"
        })
    ...
    resetDialog() {
      this.formData.value = {name: '', parentId: null};
    ...
    saveNode() {
      if (this.formData.name == '') {
        ...
        this.formData.appId = window.localStorage.getItem("Power_appId");
        this.axios.post("/job/catalog/save", this.formData).then((res) => {
          console.log(res);
          this.dialogVisible = false;
          this.formData = {name: '', parentId: null, id: null};
          this.getCatalogTree();
        });
      }
    },
    ...
    getCatalogTree() {
      this.axios.get("/job/catalog/getTree").then((res) => {
        this.treeData = [res];
